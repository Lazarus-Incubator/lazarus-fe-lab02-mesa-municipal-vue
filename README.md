...
Construir un **CRUD sencillo en Vue** que permita:
...
2. Si el último `codigo` es del mismo año, sumar +1 a `NNNN`
...
## Estructura sugerida del proyecto (Vue)
Una opción simple (Vue Router + vistas):
```
src/
  api/
    http.js
  ...
  views/
    ...
    ExpedienteForm.vue
  components/
    ExpedientesTable.vue  # opcional
    ExpedienteFormFields.vue # opcional
  App.vue
  main.js
```
...
Rutas sugeridas:
- `/expedientes` → listado
- `/expedientes/nuevo` → crear
- `/expedientes/:id/editar` → editar
